...
const QuizPage = () => {
  ...
  const [loadingProgress, setLoadingProgress] = useState(0); // Real-time loading progress
    ...
    if (loading) {
      const interval = setInterval(() => {
        setLoadingProgress((prev) => {
          if (prev >= 90) {
            clearInterval(interval); // Stop at 90% to allow final fetch completion
            return prev;
          }
          return prev + 10;
        });
  ...
  const confirmExit = () => {
    router.push("/classroom"); // Redirect to home or any other page
  };
