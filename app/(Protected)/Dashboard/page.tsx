...
export default function Dashboard() {
  ...
  const processPdf = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/savesyllabus', { method: 'POST', body: formData });
      ...
      setSessionData(prev => ({
        ...
        profile: {
          ...
          smt_details: data.data.smt_details,
        } as Profile
      }));
      if (!response.ok) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-[#0F172A] text-white relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mt-6 mb-6">
        {sessionData.profile?.smt_details &&
          Object.entries(sessionData.profile.smt_details).map(([course, topics]: [string, any], index: number) => (
